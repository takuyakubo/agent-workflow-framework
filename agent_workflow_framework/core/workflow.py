...
class AgentWorkflow:
    ...
    def add_node(self, node: AgentNode, after: str = None) -> None:
        ...
        if after:
            after_index = next(
                (i for i, n in enumerate(self.nodes) if n.node_name == after), None
            )
...
class ConditionalWorkflow(AgentWorkflow):
    ...
    def __init__(
        ...
        state_class: Type[NodeState],
        conditional_edges: Dict = None,
    ):
        """
    ...
    def _setup(self) -> None:
        ...
        for node_name, edges in self.conditional_edges.items():
            ...
            if condition_function and destinations:
                ...
                try:
                    node_index = next(
                        i for i, n in enumerate(self.nodes) if n.node_name == node_name
                    )
                ...
                logger.warning(
                    f"Invalid conditional edge specification for node '{node_name}'"
                )
    ...
    def add_conditional_edge(
        self, source: str, condition, destinations: Dict[str, str]
    ) -> None:
        """
