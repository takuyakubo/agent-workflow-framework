...
class AgentNode(Generic[T]):
    ...
    def action(self, state: T) -> T:
        ...
        try:
            ...
            return state.emit_error(f"An error occurred in node '{self.name}': {str(e)}")
    ...
    def run_with_instruction(self, input_text: str, additional_context: Optional[Dict[str, Any]] = None) -> str:
        """
    ...
    def run_with_tools(self, input_text: str, additional_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
    ...
    def _build_prompt(self, input_text: str, additional_context: Optional[Dict[str, Any]]) -> str:
        """
...
class StructuredOutputNode(AgentNode[T]):
    ...
    def run_with_schema(self, input_text: str, additional_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
    ...
    def proc(self, state: T) -> T:
        ...
        for step_name, step_function in self.steps:
            ...
            try:
                ...
                logger.error(f"Error in step '{step_name}' of node '{self.name}': {str(e)}")
                return current_state.emit_error(f"Error in step '{step_name}': {str(e)}")
